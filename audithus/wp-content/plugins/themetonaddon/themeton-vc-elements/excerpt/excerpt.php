<?php
if (!class_exists('WPBakeryShortCode_themeton_excerpt')) {
class WPBakeryShortCode_themeton_excerpt extends WPBakeryShortCode {
    ...
    protected function content( $atts, $content = null){
        ...
        $css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $css, ' ' ), 'themeton_excerpt', $atts );
        ...
        if(has_excerpt()){
            return sprintf("<h4 class='uk-article-title excerpt %s' %s>%s</h4>",$extra_class,$style,get_the_excerpt());
        }else {
            return '';
        }
    }
...
vc_map( array(
    "name" => esc_html__('Excerpt', 'themetonaddon'),
    "description" => esc_html__("Post short text", 'themetonaddon'),
    "base" => 'themeton_excerpt',

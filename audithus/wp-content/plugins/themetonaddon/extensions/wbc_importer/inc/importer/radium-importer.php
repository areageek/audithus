...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
  ...
  class Radium_Theme_Importer {
    ...
    public function set_demo_data( $file ) {
      ...
      if ( $importer_error ) {
        ...
        if ( !is_file( $file ) ) {
          ...
          $f = explode('extensions/wbc_importer/demo-data/', $file);
          $file_uri = sprintf('http://themeton.com/oneclick/mungu/%s', $f[1]);

          $demo_request = wp_remote_get($file_uri);
          if( isset($demo_request['body']) ){
            ...
            $tmp_content_path = trailingslashit($upload_dir['path']) . wp_get_theme()->template . '.xml';

            $wp_filesystem->put_contents( $tmp_content_path, $demo_request['body'], FS_CHMOD_FILE);
            
            $wp_import->import( $tmp_content_path );
          }
    ...
    public function set_demo_theme_options( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Theme options Import file could not be found. Please try again.', 'themetonaddon' ),
          '',
    ...
    function process_widget_import_file( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Widget Import file could not be found. Please try again.', 'themetonaddon' ),
          '',
          array( 'back_link' => true )
        );
      }
    ...
    public function import_widgets( $data ) {
      ...
      foreach ( $data as $sidebar_id => $widgets ) {
        ...
        if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
          ...
          $sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'themetonaddon' );
        }
        ...
        foreach ( $widgets as $widget_instance_id => $widget ) {
          ...
          if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
            ...
            $widget_message = __( 'Site does not support widget', 'themetonaddon' ); // explain why widget not imported
          }
          ...
          if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
            ...
            foreach ( $single_widget_instances as $check_id => $check_widget ) {
              ...
              if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
                ...
                $widget_message = __( 'Widget already exists', 'themetonaddon' ); // explain why widget not imported
          ...
          if ( ! $fail ) {
            ...
            if ( $sidebar_available ) {
              ...
              $widget_message = __( 'Imported', 'themetonaddon' );
            } else {
              $widget_message_type = 'warning';
              $widget_message = __( 'Imported to Inactive', 'themetonaddon' );
            }
          ...
          $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'themetonaddon' ); // show "No Title" if widget instance is untitled

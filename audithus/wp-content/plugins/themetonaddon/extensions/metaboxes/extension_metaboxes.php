    ...
    if ( ! class_exists( 'ReduxFramework_extension_metaboxes' ) ) {
        ...
        class ReduxFramework_extension_metaboxes {
                        ...
                        foreach ( $box['post_types'] as $posttype ) {
                            if ( isset( $box['title'] ) ) {
                                ...
                                if ( isset( $box['sections'] ) && count( $box['sections'] ) == 1 && isset( $box['sections'][0]['fields'] ) && count( $box['sections'][0]['fields'] ) == 1 && isset( $box['sections'][0]['fields'][0]['title'] ) ) {
                                    ...
                                    $title = ucfirst( $posttype ) . " " . esc_html__( 'Options', 'themetonaddon' );
                                }
            ...
            function generate_boxes( $post, $metabox ) {
                ...
                ?>
                <div class="redux-container<?php echo ( $sidebar ) ? ' redux-has-sections' : ' redux-no-sections'; ?> redux-box-<?php echo esc_attr($metabox['args']['position']); ?>">
                    <div class="redux-notices">
                        <?php if ( $sidebar ) { ?>
                            <div class="saved_notice admin-notice notice-blue" style="display:none;">
                                <strong><?php echo apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'themetonaddon' ) ) ?></strong>
                            </div>
                            <div class="redux-save-warn notice-yellow">
                                <strong><?php echo apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'themetonaddon' ) ) ?></strong>
                            </div>
                        <?php } ?>
                        <div class="redux-field-errors notice-red">
                            <strong>
                                <span></span> <?php echo esc_html__( 'error(s) were found!', 'themetonaddon' ) ?>
                            </strong>
                        </div>
                        <div class="redux-field-warnings notice-yellow">
                            <strong>
                                <span></span> <?php echo esc_html__( 'warning(s) were found!', 'themetonaddon' ) ?>
                            </strong>
                        </div>
                    </div>
                    <?php
                    echo '<a href="javascript:void(0);" class="expand_options hide" style="display:none;">' . esc_html__( 'Expand', 'themetonaddon' ) . '</a>';
                        if ( $sidebar ) {
                            ?>
                                ...
                                <ul class="redux-group-menu">
                                    <?php
                                        foreach ( $sections as $sKey => $section ) {
                                            print $this->parent->section_menu( $sKey, $section, '_box_' . $metabox['id'], $sections );
                                        }
                                    ?>
                                </ul>
                            </div>
...
?>
            ...
            function meta_boxes_show_errors() {
                if ( isset( $this->notices['errors'] ) && ! empty( $this->notices['errors'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['errors'] ) . ' ' . esc_html__( 'error(s) were found!', 'themetonaddon' ) . '</strong></p>';
                    echo '</div>';
                }
                ...
                if ( isset( $this->notices['warnings'] ) && ! empty( $this->notices['warnings'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['warnings'] ) . ' ' . esc_html__( 'warnings(s) were found!', 'themetonaddon' ) . '</strong></p>';
                    echo '</div>';
                }

...
class Themeton_Less{
    ...
    public function create_css( $modify_vars=false ){
        ...
        $css_file = array(
            'path' => trailingslashit($upload_dir['path']) . get_template().'.css',
            'url'  => trailingslashit($upload_dir['url']) . get_template().'.css'
        );
        if(is_ssl()) {
    ...
    public function generate_css( $modify_vars=false ){
        ...
        try{
            ...
            if ( class_exists('TT_Post_type_PT') ) {
                ...
                foreach( $vc_elements_all_files as $vc_elem_file ){
                    ...
                    if( $vc_files_ext=='less' ){
                        $vc_element_less_content[] = Themeton_Std::fs_get_contents($vc_elem_file);
                    }
                }
            }
            ...
            foreach( $vc_elements_all_files as $vc_elem_file ){
                ...
                if( $vc_files_ext=='less' && strpos($vc_elem_file, 'skin-'.ThemetonStd::getopt('medio_skin'))){
                    $vc_element_less_content[] = Themeton_Std::fs_get_contents($vc_elem_file);
                }
            }
...
class Themeton_Init_Custom_CSS{
    function __construct(){
        add_action( 'wp_enqueue_scripts', array($this, 'medio_enqueue_script') );
    }

    public function medio_enqueue_script(){
        // theme customized style
        $less_css_path = Themeton_Less::get_less_path();

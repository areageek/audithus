...
class WPSEO_Options_Extend {
    ...
    public static function get_all() {
        ...
        $all_options['breadcrumbs-404crumb']      = esc_html__('Error 404: Page not found', 'medio');
        $all_options['breadcrumbs-archiveprefix'] = esc_html__('Archives for', 'medio');
        $all_options['breadcrumbs-home']          = esc_html__('Home', 'medio');
        $all_options['breadcrumbs-searchprefix']  = esc_html__('You searched for', 'medio');

        if( is_home() && get_query_var('post_type')=='portfolio' || is_singular('portfolio') || get_query_var('portfolio_entries')!='' ){
            $all_options['breadcrumbs-home'] = Themeton_Std::get_mod('portfolio_label');
        }
...
class WPSEO_Breadcrumbs_Extend {
    ...
    private function prepare_links() {
        ...
        foreach ( $this->crumbs as $i => $crumb ) {
            if (count($this->crumbs) == $counter) break;
            $counter++;
